...
1. В Бинарное дерево добавить следующие методы:
	...
	??? erase(???);		DONE
	??? depth(???);		DONE
	??? depth_print(???);	//выводит все элементы дерева на заданонй глубине
	??? tree_print(???);	//выводит дерево как дерево
...
3. Измерить производительность всех методов:
	https://legacy.cplusplus.com/reference/ctime/
4. Код измерения производительности методов вынести в функцию ??? measure(???);
